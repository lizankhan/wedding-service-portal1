...
if(empty($name) || empty($email) || empty($phone)  ) {
  ...
  Redirect_to("update_client_info.php?id=$searchquery;");

}else {
   //Query to update info into database
   global $connectingdb;
   if(!empty($_FILES["image"]["name"])){
     $sql="UPDATE  client SET name='$name',email='$email',password='$conpassword',phone='$phone',type='$type',image='$image'
           WHERE id='$searchquery'";
   }else {
     $sql="UPDATE  client SET name='$name',email='$email',password='$conpassword',phone='$phone',type='$type'
           WHERE id='$searchquery'";
   }
    ...
    <div class="row">
      <div class="col-lg-6">
        <?php
         ...
         ?>
        <form class="form-group" action="update_client_info.php?id=<?php echo $searchquery; ?>" method="post" enctype="multipart/form-data"> <!--enctype for image extraction-->
          <!--
          ...
          <input type="password" name="password" class="form-control" value="<?php echo $passwordToBeUpdated?>">
          </div>
          <div class="form-group">
...
<section id="copyright" class="text-center py-3 text-light">
  <div class="container">
    <div class="row">
      <div class="col">
        <p class="lead mb-0">Copyright 2019 &copy; ALS</p>
      </div>
    </div>
  </div>
</section>
